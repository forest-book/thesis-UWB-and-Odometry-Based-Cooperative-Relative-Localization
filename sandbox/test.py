...
class Estimator:
    ...
    def fused_estimate(self, uav_i, target_j_id, all_uavs, kappa_D, kappa_I, noisy_v_ij, dt):
        ...
        for r_id in uav_i.neighbors:
            ...
            # 間接推定値 x_hat_rj_indirect = x_hat_ir + pi_rj
...
class Environment:
    ...
    def _setup_scenario(self):
        ...
        # 全てのUAVが全てのUAVに対して推定値を初期化(0で)
        for uav in self.uavs:
            for target_uav in self.uavs:
                uav.direct_estimates[target_uav.id] = np.zeros(2)
                uav.fused_estimates[target_uav.id] = np.zeros(2)
    ...
    def _calculate_kappas(self, uav_i, target_j_id):
        ...
        num_neighbors = len(uav_i.neighbors)
        is_direct = target_j_id in uav_i.neighbors
        alpha_ij = 1 if is_direct else 0
        
        denominator = num_neighbors + 1 + alpha_ij
        kappa_D = alpha_ij / denominator
        kappa_I = 1 / denominator # 論文ではkappa_ir^Iだが、rによらない定数として実装
    ...
    def run_step(self):
        ...
        # 現ステップの計算結果を一時保存（次ステップで一斉に反映）
        next_direct_estimates = defaultdict(dict)
        next_fused_estimates = defaultdict(dict)

        # 2. 推定計算を実行
        target_j = self.uavs[self.params['target_id'] - 1]

        for uav_i in self.uavs:
            if uav_i.id == target_j.id:
                continue

            # --- 直接推定の計算 ---
            # センシンググラフで直接繋がっている全ペアについて計算
            ...
            # --- 融合推定の計算 (ターゲットはUAV1) ---
            kappa_D, kappa_I = self._calculate_kappas(uav_i, target_j.id)
            noisy_v_ij, _, _ = self.get_noisy_measurements(uav_i, target_j)

            next_fused = self.estimator.fused_estimate(uav_i, target_j.id, self.uavs, kappa_D, kappa_I, noisy_v_ij, self.params['dt'])
            next_fused_estimates[uav_i.id][target_j.id] = next_fused
        ...
        for uav in self.uavs:
            ...
            if uav.id in next_fused_estimates:
                 # 他のターゲットへの推定値も維持するため、ターゲットIDの分だけ更新
                 if self.params['target_id'] in next_fused_estimates[uav.id]:
                    uav.fused_estimates[self.params['target_id']] = next_fused_estimates[uav.id][self.params['target_id']]
        ...
        for uav in self.uavs:
            ...
            if uav.id != target_j.id:
                true_relative_pos = uav.true_position - true_pos_uav1
                fused_estimate = uav.fused_estimates[target_j.id]
    ...
    def plot_results(self):
        ...
        for i in range(2, 7):
            plt.plot(self.history['time'], self.history[f'uav{i}_fused_error'], label=f'||π_{i}1 - x_{i}1||')
        
        plt.title('Fused RL Estimation Error (Target: UAV1)')
        plt.xlabel('Time (s)')
        plt.ylabel('Error Norm (m)')
        plt.ylim(0, 1.0) # 論文のスケールに合わせる
        plt.legend()
        plt.grid(True)
        plt.show()
        ...
        for i in range(2, 7):
            ...
            # 開始直後の過渡状態を除外して計算 (e.g., 10秒後から)
            transient_steps = int(10 / self.params['dt'])
            stable_errors = errors[transient_steps:]
...
if __name__ == '__main__':
    # 論文V-A節に基づくシミュレーションパラメータ
    simulation_params = {
        'dt': 0.05,  # サンプリング周期 T (20 Hz)
        'gamma': 0.5, # ゲイン
        'delta_bar': 0.5, # 速度ノイズの上限
        'target_id': 1, # 推定対象
        'initial_positions': {
        ...
        # シナリオを選択: 'continuous' または 'sudden_turn'
        'event': 'sudden_turn' 
    }
